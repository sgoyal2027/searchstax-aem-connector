...
public class SearchStaxFullIndexStatusServlet extends SlingAllMethodsServlet {
    ...
    protected void doGet(final SlingHttpServletRequest request, final SlingHttpServletResponse response)
        ...
        final boolean complete = !hasActiveOrQueuedJob && terminalSnapshot;
        final boolean running = hasActiveOrQueuedJob || snapshotState == State.RUNNING;
        final State state = running && !complete ? State.RUNNING : snapshotState;
        final boolean currentRunInProgress = snapshotState == State.RUNNING;
        final boolean pendingStart = hasActiveOrQueuedJob && snapshotState == State.IDLE;
        final boolean exposeProgress = currentRunInProgress || complete;
        final long startedAt =
                exposeProgress && (currentRunInProgress || complete) ? progress.getStartedAt() : 0L;
        final long elapsedMs =
                currentRunInProgress ? progress.getElapsedMs() : (complete ? progress.getElapsedMs() : 0L);
        final String statusMessage =
                pendingStart
                        ? "Waiting for full index job to start..."
                        : (running
                                ? (currentRunInProgress ? progress.getMessage() : "Full index job is running.")
                                : progress.getMessage());

        final Map<String, Object> body = new LinkedHashMap<>();
        body.put("jobId", currentJobId);
        body.put("state", state.name());
        body.put("message", statusMessage);
        ...
        body.put("pendingStart", pendingStart);
